...
| 文件 | 用途 | 谁来改 |
|---|---|---|
| `docker-compose.yaml` | **预合并生成文件**：上游 compose + Portainer override 合并后的最终结果，Portainer 直接部署它（`${VAR}` 变量保留，仍由 Portainer 的 stack env 注入） | **只由机器人生成**，手改会被覆盖 |
...
2. 与本分支的 override 合并（`docker compose config --no-interpolate`）生成 `docker-compose.yaml`
3. 校验合并结果（自解析 + 关键配置哨兵检查），通过且有变化才提交推送
